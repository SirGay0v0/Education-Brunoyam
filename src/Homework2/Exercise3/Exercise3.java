...
public class Exercise3 {
    public static void main(String[] args) {
        ...
        for (i = 0; i < first; i++) {
            for (j = 0; j < second; j++) {
                if (j == second - 1 & i < first - 1) {Array[i][j] = Array[i + 1][0];} //Если это последний элемент строки (за исключением последней строки), то меняем его на первый элемент следующей строки
                else if (i == first - 1 & j == second - 1) Array[first-1][second-1] = helpSpace; //Если это последний элемент последней строки, то меняем его на значение первого элемента первой строки (ранее записанного в helpSpace)
